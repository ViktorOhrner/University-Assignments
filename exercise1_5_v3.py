...
class Net(nn.Module):
    ...
    def forward(self, x):
        in_size = x.size(0)
        x = F.hardtanh(F.max_pool2d(self.conv1(x), 2, stride=(2, 2)))
        x = F.hardtanh(F.max_pool2d(self.conv2(x), 2, stride=(2, 2)))
...
if __name__ == "__main__":
    ...
    print("Train:")
    print(f"Accuracy: {acc_train[-1]}")
    print(f"Cost: {loss_train[-1]}")
